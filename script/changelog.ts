...
export async function getLatestRelease() {
  return fetch("https://api.github.com/repos/arihantchoudhary/stardrop/releases/latest")
    .then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/arihantchoudhary/stardrop/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/stardrop packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/stardrop/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/stardrop/")) areas.add("core")
...
async function summarizeCommit(opencode: Awaited<ReturnType<typeof createStardrop>>, message: string): Promise<string> {
...
export async function generateChangelog(commits: Commit[], opencode: Awaited<ReturnType<typeof createStardrop>>) {
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/arihantchoudhary/stardrop/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
