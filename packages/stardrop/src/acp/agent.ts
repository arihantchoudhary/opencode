...
import type { Event, StardropClient, SessionMessageResponse } from "stardrop-sdk/v2"
import { applyPatch } from "diff"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk: _sdk }: { sdk: StardropClient }) {
    return {
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const opencodeProvider = providers.find((p) => p.id === "stardrop")
    if (opencodeProvider) {
      if (opencodeProvider.models["big-pickle"]) {
        return { providerID: "stardrop", modelID: "big-pickle" }
      }
