...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g stardrop",
      pnpm: "pnpm uninstall -g stardrop",
      bun: "bun remove -g stardrop",
      yarn: "yarn global remove stardrop",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "stardrop"],
      yarn: ["yarn", "global", "remove", "stardrop"],
      brew: ["brew", "uninstall", "stardrop"],
      choco: ["choco", "uninstall", "stardrop"],
      scoop: ["scoop", "uninstall", "stardrop"],
    }
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# opencode") || content.includes(".stardrop/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".stardrop/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".stardrop/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".stardrop"))
    ) {
      continue
    }
