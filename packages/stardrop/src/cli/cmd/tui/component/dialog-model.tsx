...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "stardrop" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "stardrop" && model.id.includes("-nano"),
              footer: model.cost?.input === 0 && provider.id === "stardrop" ? "Free" : undefined,
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "stardrop" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "stardrop" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
