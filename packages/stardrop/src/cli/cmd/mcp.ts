    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in stardrop.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .stardrop/ subdirectory too)
  const candidates = [path.join(baseDir, "stardrop.json"), path.join(baseDir, "stardrop.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".stardrop", "stardrop.json"), path.join(baseDir, ".stardrop", "stardrop.jsonc"))
  }
  ...
  // Default to stardrop.json if none exist
  return candidates[0]
}
