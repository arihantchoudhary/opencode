...
const AGENT_USERNAME = "stardrop-cli"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/stardrop.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://stardrop.dev/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.stardrop.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
  ...
  stardrop:
    if: |
      (github.event_name == 'issue_comment' && (
        contains(github.event.comment.body, '@stardrop-cli') ||
        contains(github.event.comment.body, '/stardrop') ||
        contains(github.event.comment.body, ' /sd') ||
        startsWith(github.event.comment.body, '/sd')
      )) ||
      ...
      (github.event_name == 'issues' && github.event.action == 'labeled' && github.event.label.name == 'stardrop')
    ...
    steps:
      ...
      - name: Configure git identity
        run: |
          git config user.name "Stardrop"
          git config user.email "rosemaryrunner@icloud.com"

      - name: Install Stardrop
        run: npm i -g stardrop || curl -fsSL https://opencode.ai/install | bash

      - name: Add to PATH
        run: echo "$$HOME/.opencode/bin" >> $$GITHUB_PATH

      - name: Run Stardrop agent
        env:
          MODEL: ${provider}/${model}
          GITHUB_TOKEN: \${{ secrets.GITHUB_TOKEN }}
          USE_GITHUB_TOKEN: "true"${envStr}
        run: stardrop github run`,
            )
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.stardrop.dev" : "https://stardrop.dev"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.stardrop.dev"
