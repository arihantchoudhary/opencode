...
import { createStardropClient, type StardropClient } from "stardrop-sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: StardropClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createStardropClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.STARDROP_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createStardropClient({ baseUrl: "http://stardrop.internal", fetch: fetchFn })
