...
declare global {
  const STARDROP_VERSION: string
  const STARDROP_CHANNEL: string
}
...
export namespace Installation {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".stardrop", "bin"))) return "curl"
    ...
    for (const check of checks) {
      ...
      const installedName =
        check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "stardrop" : "stardrop"
  ...
  async function getBrewFormula() {
    ...
    if (tapFormula.includes("stardrop")) return "anomalyco/tap/opencode"
    const coreFormula = await $`brew list --formula opencode`.throws(false).quiet().text()
    if (coreFormula.includes("stardrop")) return "stardrop"
    return "stardrop"
  }
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        cmd = $`curl -fsSL https://stardrop.dev/install | bash`.env({
          ...process.env,
          VERSION: target,
        })
        break
      case "npm":
        cmd = $`npm install -g stardrop@${target}`
        break
      case "pnpm":
        cmd = $`pnpm install -g stardrop@${target}`
        break
      case "bun":
        cmd = $`bun install -g stardrop@${target}`
        break
      case "brew": {
  ...
  export async function latest(installMethod?: Method) {
    ...
    if (detectedMethod === "brew") {
      const formula = await getBrewFormula()
      if (formula === "stardrop") {
        return fetch("https://formulae.brew.sh/api/formula/stardrop.json")
          .then((res) => {
    ...
    if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
      ...
      return fetch(`${registry}/stardrop/${channel}`)
        .then((res) => {
          if (!res.ok) throw new Error(res.statusText)
          return res.json()
        })
    ...
    if (detectedMethod === "scoop") {
      return fetch("https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/stardrop.json", {
