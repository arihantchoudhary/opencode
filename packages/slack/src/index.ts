...
import { createStardrop, type ToolPart } from "stardrop-sdk"
...
console.log("🚀 Starting stardrop server...")
const stardrop = await createStardrop({
  port: 0,
})
console.log("✅ Stardrop server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await stardrop.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new stardrop session...")
    const { client, server } = stardrop
    ...
    console.log("✅ Created stardrop session:", createResult.data.id)
  ...
  console.log("📝 Sending to stardrop:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Stardrop response:", JSON.stringify(result, null, 2))
